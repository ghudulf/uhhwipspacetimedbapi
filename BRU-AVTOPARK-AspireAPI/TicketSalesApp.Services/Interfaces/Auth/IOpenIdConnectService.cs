...
namespace TicketSalesApp.Services.Interfaces
{
    ...
    public interface IOpenIdConnectService
    {
        ...
        Task<(bool success, List<object>? authorizations, string? errorMessage)> GetAuthorizationsAsync(string subject, object application, string status, string type, string[] scopes);
        Task<(bool success, ClaimsIdentity? identity, string? errorMessage)> CreateIdentityFromUserAsync(UserProfile user, string[] scopes);
        Task<(bool success, object? authorization, string? errorMessage)> CreateAuthorizationAsync(ClaimsIdentity identity, string subject, object application, string type, string[] scopes);
        Task<(bool success, string? id, string? errorMessage)> GetAuthorizationIdAsync(object authorization);
        Task<(bool success, List<string>? resources, string? errorMessage)> GetResourcesAsync(string[] scopes);
        ...
        IOpenIddictApplicationManager GetApplicationManager();
    }
}
