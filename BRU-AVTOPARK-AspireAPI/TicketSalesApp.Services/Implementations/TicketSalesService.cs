...
namespace TicketSalesApp.Services.Implementations
{
    public class TicketSalesService : ITicketSalesService
    {
        ...
        public async Task<(List<Sale> items, int totalCount)> GetSalesPageAsync(int page, int pageSize)
        {
            try
            {
                _logger.LogInformation("Retrieving sales page {Page} with page size {PageSize}", page, pageSize);
                ...
                var conn = _spacetimeService.GetConnection();
                var allItems = conn.Db.Sale.Iter().ToList();
                var totalCount = allItems.Count;
                var items = allItems.Skip((page - 1) * pageSize).Take(pageSize).ToList();

                _logger.LogInformation("Retrieved {ItemCount} sales out of {TotalCount} total (page {Page})", items.Count, totalCount, page);
                return (items, totalCount);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving sales page {Page}", page);
                throw;
            }
        }
