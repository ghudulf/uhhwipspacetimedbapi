...
5. WHERE MAUI_Integration is adopted, the `App.axaml.cs` `RegisterServices()` method SHALL call `AvaloniaWebViewBuilder.Initialize(default)` only under a DESKTOP-only build guard (`#if DESKTOP`), ensuring projects migrating to `Avalonia.Controls.WebView` do not invoke the legacy initializer on MAUI targets where the desktop `NativeWebView` is unavailable.
