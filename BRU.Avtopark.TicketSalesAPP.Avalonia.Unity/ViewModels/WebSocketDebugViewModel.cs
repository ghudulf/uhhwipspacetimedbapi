...
public partial class WebSocketDebugViewModel : ObservableObject
{
    ...
    private readonly HashSet<string> _subscribedResources = new();
    ...
    private async Task ConnectWebSocket()
    {
        try
        {
            ...
            // Reset reconnection state for user-initiated connection
            ...
            if (_isReconnecting && _subscribedResources.Count > 0)
            {
                AddLog($"🔄 Restoring {_subscribedResources.Count} subscription(s)...");
                foreach (var resource in _subscribedResources.ToList())
                {
                    await RestoreSubscription(resource);
                }
            ...
            _ = Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    // Trigger reconnection if not intentional disconnect
        ...
        catch (Exception ex)
        {
            ...
            // Trigger reconnection if this was not an intentional disconnect
            if (!_isIntentionalDisconnect && AutoReconnect && _reconnectAttempts < MaxReconnectAttempts)
            {
                _ = AttemptReconnect();
            }
        }
    ...
    private async Task SubscribeToResource(string resource)
    {
        ...
        try
        {
            ...
            _subscribedResources.Add(resource);
    ...
    private async Task UnsubscribeFromResource(string resource)
    {
        ...
        try
        {
            ...
            _subscribedResources.Remove(resource);
    ...
    {
        if (_isIntentionalDisconnect || _isReconnecting)
        {
            return; // Don't reconnect if user disconnected intentionally or already reconnecting
        }

        _isReconnecting = true;

        while (_reconnectAttempts < MaxReconnectAttempts && !_isIntentionalDisconnect)
        {
            _reconnectAttempts++;

            // Calculate exponential backoff delay
            var delayMs = InitialReconnectDelayMs * Math.Pow(2, _reconnectAttempts - 1);
            var delay = TimeSpan.FromMilliseconds(Math.Min(delayMs, 30000)); // Cap at 30 seconds

            StatusMessage = $"Reconnecting in {delay.TotalSeconds:F1}s (attempt {_reconnectAttempts}/{MaxReconnectAttempts})";
            AddLog($"🔄 Reconnection attempt {_reconnectAttempts}/{MaxReconnectAttempts} in {delay.TotalSeconds:F1}s...");
            ...
            try
            {
                StatusMessage = $"Reconnecting... (attempt {_reconnectAttempts}/{MaxReconnectAttempts})";
                AddLog($"🔄 Attempting to reconnect (attempt {_reconnectAttempts}/{MaxReconnectAttempts})...");
                ...
                // Try to reconnect using the same logic as ConnectWebSocket
                var tokens = await _tokenStorage.GetTokensAsync();
                if (tokens == null || string.IsNullOrEmpty(tokens.AccessToken))
                {
                    AddLog("✗ No access token available for reconnection. Please login.");
                    StatusMessage = "Reconnection failed: Authentication required";
                    _isReconnecting = false;
                    return;
                }
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        AddLog($"✗ ReceiveMessagesAsync failed after reconnect: {ex.Message}");
                        Log.Error(ex, "ReceiveMessagesAsync failed after reconnect");
            ...
            {
                AddLog($"✗ Reconnection attempt {_reconnectAttempts} failed: {ex.Message}");
                StatusMessage = $"Reconnection attempt {_reconnectAttempts} failed";

                if (_reconnectAttempts >= MaxReconnectAttempts)
                {
                    AddLog($"✗ Max reconnection attempts ({MaxReconnectAttempts}) reached. Giving up.");
                    StatusMessage = "Reconnection failed: Max attempts reached";
                    _isReconnecting = false;
                    return;
                }
            }
    ...
    private async Task RestoreSubscription(string resource)
    {
        try
        {
            if (_webSocket?.State != WebSocketState.Open)
            {
                AddLog($"⚠ Cannot restore subscription for {resource}: WebSocket not open");
                return;
            }
            ...
            await _webSocket.SendAsync(bytes, WebSocketMessageType.Text, true, _cts?.Token ?? CancellationToken.None);
            AddLog($"✓ Restored subscription for resource: {resource}");
        }
        catch (Exception ex)
        {
            AddLog($"✗ Failed to restore subscription for {resource}: {ex.Message}");
        }
    }
