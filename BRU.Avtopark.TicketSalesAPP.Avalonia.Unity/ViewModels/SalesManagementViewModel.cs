...
namespace BRU.Avtopark.TicketSalesAPP.Avalonia.Unity.ViewModels
{
    public partial class SalesManagementViewModel : ReactiveObject
    {
        ...
        public SalesManagementViewModel()
        {
            ...
            {
                Log.Information("Token already set in SalesManagementViewModel constructor, loading data");
                LoadData().ConfigureAwait(false);
            }
            else
            {
                Log.Warning("Token not set in SalesManagementViewModel constructor, waiting for OnAuthTokenChanged event");
            }
        }
